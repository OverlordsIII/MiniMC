...
public class VCUnmuteCommand implements TextCommand {
	@Override
	public void execute(MessageReceivedEvent event) {
		...
		if (channel != null) {
			channel.getMembers().forEach(member1 -> member1.mute(false).queue());
			MessageEmbed embed = new EmbedCreator()
				.setColor(Color.GREEN)
				.setTitle("Unmuted Users!")
				.addField("Unmuted " + channel.getMembers().size() + " Users!", channel.getMembers()
					.stream()
